...
| `tags`, `document_tags` | Relational junction for multi-tag assignment. A tag keeps its colour and icon on its own row, where a rename, merge or move cannot orphan them. Tags nest up to five deep, and assigning a child attaches every ancestor as `implied`, a row that goes again once no tag of the document's own sits below it. A tag's sidebar count is the documents its row lists: those in the Trash or missing from disk are not counted |
