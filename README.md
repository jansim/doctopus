...
- Secondary Folder Presence (Finder Aliases): While every document has exactly one physical master location on disk, membership in additional folders/tags can optionally mirror to disk by automatically generating native macOS Finder Aliases (.alias / bookmark resolution), configurable globally or per-tag. Deleting a document that was filed in another folder by hand is read as leaving that folder rather than as a deletion: the closest of those aliases is taken away and the master moved into its place, so the document stays in the library and the other placements keep pointing at a real file.
...
- **Nothing moves uninvited** — the only thing Doctopus ever moves or rewrites on its own is a file it has just brought in itself (a scan, or the copy an import makes), and it only moves one when nobody chose a folder for it. Concretely:
  ...
  - Doctopus only deletes an alias it made for a tag, and only if the file at that path is still an alias to that document. An alias you made by dragging onto a folder is only ever taken away to put the document itself in its place, when that document is deleted from where it lived.
  - Move to Trash uses the Trash (never a hard delete), keeps the index entry if trashing fails, and asks first when the row is only an alias in the folder being viewed. A document that is also filed in another folder by hand does not reach the Trash at all: it moves to the nearest of those folders, which takes the alias's place. Tag mirrors in `Tags/` are not promoted — they are a view of the library, not a home — and neither is a placement outside the library root.
