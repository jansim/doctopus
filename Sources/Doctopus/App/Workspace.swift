...
final class Workspace {
    ...
    /// Libraries reopened at launch, which open without a toast.
    @ObservationIgnored private var quiet: Set<URL> = []
    ...
    /// Whether this open is a launch reopening, which is not worth a toast.
    func takeQuiet(_ container: URL) -> Bool {
        quiet.remove(container.standardizedFileURL) != nil
    }

    /// Launch opens a single empty window. It takes the first library that was
    /// open last time, and each of the others gets a window of its own.
    func restore(into first: AppModel) async {
        ...
        for bookmark in Preferences.libraryBookmarks {
            ...
            containers.append((container, bookmark))
        }
        quiet = Set(containers.map { $0.container.standardizedFileURL })
        restoring = true
    ...
    func persistOpenLibraries() {
        ...
        Preferences.libraryBookmarks = windows.compactMap { $0.library?.bookmark }
    }
